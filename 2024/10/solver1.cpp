...
int calculate_score(pair<size_t, size_t> &head, vector<vector<int>> &grid, unordered_map<size_t, unordered_set<size_t>> &seen_peaks);
...
int main()
{
    ...
    {
        unordered_map<size_t, unordered_set<size_t>> seen_peaks;
        score += calculate_score(trailhead, grid, seen_peaks);
    }
    cout << score << endl;
}
...
int calculate_score(pair<size_t, size_t> &head, vector<vector<int>> &grid, unordered_map<size_t, unordered_set<size_t>> &seen_peaks)
{
    ...
    if (grid[y][x] == 9 && seen_peaks[x].find(y) == seen_peaks[x].end())
    {
        seen_peaks[x].insert(y);
        return 1;
    }
    ...
    for (pair<size_t, size_t> coords: ds)
    {
        ...
        {
            sum += calculate_score(coords, grid, seen_peaks);
        }
    }
